...
describe('strings-tasks', () => {
  ...
  it.optional('removeTrailingWhitespaces should remove trailing whitespaces from the string', () => {
    ...
    assert.equal(tasks.removeTrailingWhitespaces('\tHello, World! '), '\tHello, World!');
    assert.equal(tasks.removeTrailingWhitespaces('\t\t\tHello, World! '), '\t\t\tHello, World!');
    assert.equal(tasks.removeTrailingWhitespaces(' Hello, World!\t\t\t'), ' Hello, World!');
    assert.equal(tasks.removeTrailingWhitespaces(''), '');
    assert.equal(tasks.removeTrailingWhitespaces('    '), '');
  ...
  it.optional('removeFirstOccurrences should remove the first occurrence of a substring from a string', () => {
    assert.equal(tasks.removeFirstOccurrences('To be or not to be', 'be'), 'To  or not to be');
    ...
    assert.equal(tasks.removeFirstOccurrences('Hello, World!', ''), 'Hello, World!');
    assert.equal(tasks.removeFirstOccurrences('The quick brown fox', 'dog'), 'The quick brown fox');
  ...
  it.optional('formatTime should format time with leading zeros for minutes and seconds', () => {
  ...
  it.optional('orderAlphabetically should order characters in alphabetical order for a given string', () => {
  ...
  it.optional('containsSubstring should return true if the input string contains the specified substring', () => {
    ...
    assert.equal(tasks.containsSubstring('JavaScript is Fun', 'Python'), false);
  ...
  it.optional('countVowels should return the number of vowels in the string', () => {
  ...
  it.optional('isPalindrome should return true if the string is a palindrome', () => {
  ...
  it.optional('findLongestWord should return the longest word in the sentence', () => {
    ...
    assert.equal(tasks.findLongestWord('A long and winding road winding'), 'winding');
    assert.equal(tasks.findLongestWord('The QuIck BROWN foX'), 'QuIck');
  ...
  it.optional('reverseWords should reverse each word in the sentence', () => {
    ...
    assert.equal(tasks.reverseWords('The Quick Brown Fox'), 'ehT kciuQ nworB xoF');
    assert.equal(tasks.reverseWords('12345'), '54321');
    assert.equal(tasks.reverseWords('UPPERCASE lowercase'), 'ESACREPPU esacrewol');
    assert.equal(tasks.reverseWords(''), '');
  });

  it.optional('invertCase should return the string with the case of each character inverted.', () => {
